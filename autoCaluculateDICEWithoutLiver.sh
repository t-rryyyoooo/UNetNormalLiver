...
echo -n "Is json file name caluculateDICEWithoutLiver.json?[y/n]:"
read which
...
if [ $which = "y" ];then
 JSON_NAME="caluculateDICEWithoutLiver.json"
else
...
python3 caluculateDICEWithoutLiver.py ${TRUE_DIRECTORY} ${PREDICT_DIRECTORY} ${SAVE_PATH} ${PATIENTID_LIST} --classes ${CLASSES} --class_label ${CLASS_LABEL} --true_name ${TRUE_NAME} --predict_name ${PREDICT_NAME} 
